...
namespace Gauss_1
{
    internal class Interblock_inversions
    {
        ...
        {
          if(input < 0 || firstBlockSize < 0 || secondBlockSize < 0)
          {
        ...
        public (List<List<int>>, List<List<string>>) Count_of_inversions(int required_quantity)
        {
            ...
            if(required_quantity > size_int_1 * (size - size_int_1))
            {
            ...
            int the_number_of_possible_permutations_with_k_inversions = Math.Abs(result[required_quantity]);
            ...
            int temp_of_count_interversions_all_types = 0;
            int imp = 0;
